...
export const DEVELOPMENT = [
  {
    ...
    refs: ['#14'],
    changes: [
      { category: 'added', text: '`npm run make` produces genuine artifacts: WinSCP Material 0.1.0 Setup.exe (124.86 MB, PE32 GUI executable), winscp_material-0.1.0-full.nupkg (123.51 MB), and a RELEASES file carrying the SHA1 and size Squirrel expects.' },
      { category: 'fixed', text: 'extract-zip 2.0.1 unpacked exactly one entry and then never settled its promise, so the process exited 0 with nothing to show for it — the same fault that stopped Electron’s own postinstall.' },
      { category: 'fixed', text: 'cross-zip called fs.rmdir(path, { recursive: true }), which Node 26 removed outright.' },
      { category: 'added', text: 'tools/fix-node26-deps.js patches both packages, verifies the fix by actually unpacking the Electron zip (21 entries in 2.0 s where the original hung), and refuses to silently succeed if upstream source has moved under it. It touches node_modules only.' },
      { category: 'added', text: 'Electron Forge config, the CI workflow, README and AGENTS with a sanitized mirror of the shared instructions, LICENSE, CONTRIBUTING, SECURITY and CODE_OF_CONDUCT, categorized docs, and the Material 3 landing site.' },
    ],
  },
  {
    ...
    refs: ['#6', '#12', '#13', '#15'],
    changes: [
      { category: 'added', text: 'First real launch, captured from the running window: custom title bar, tab strip, status bar, the language chip, both funny-level indicators, the search bar with its regex builder, and the release code name.' },
      { category: 'added', text: 'Main process with 130 IPC channels across app, config, session, fs, queue, sync, editor and history — every handler resolving { ok, value } or { ok, error } and never throwing across the bridge.' },
      { category: 'security', text: 'Preload exposes no generic invoke and no Node primitives.' },
      { category: 'security', text: 'Host key and certificate verification always ask the renderer and await the answer. There is no auto-accept path, and a prompt that times out resolves to CANCEL rather than to yes.' },
    ],
  },
  {
    ...
    refs: ['#2', '#3', '#4', '#5'],
    changes: [
      { category: 'added', text: 'docs/protocol-gaps.md counts seven capabilities PuTTY has and ssh2 does not, each row stating what the user cannot do rather than which function is missing from which library.' },
      { category: 'changed', text: 'GSSAPI/Kerberos has no ssh2 mechanism at all: the flags are accepted and a warning is logged, and single sign-on genuinely does not work.' },
      { category: 'changed', text: 'ssh2’s client hard-codes SFTP version 3, so the version selector and every v4+ bug workaround are inert.' },
      { category: 'changed', text: 'Request pipelining is confined to fastGet/fastPut, which cannot resume or report progress, so sftpDownloadQueue and sftpUploadQueue do nothing on the streaming path.' },
      { category: 'added', text: '17 tests for find.js, including a 3-byte-chunk reader proving a match spanning a read boundary is still found, and an assertion that an oversized file is never opened.' },
      { category: 'fixed', text: 'queue.parallelTransfers and parallelTransferThreshold were read by the queue but absent from the defaults, so the feature worked and no user could reach it.' },
    ],
  },
  {
    ...
    refs: ['#3'],
    changes: [
      { category: 'added', text: 'FTP active mode (PORT/EPRT) written by hand, because basic-ftp is passive-only and wants a live data socket before the command goes out; RFC 4217’s rule that the client stays the TLS client on a server-opened connection is honoured.' },
      { category: 'added', text: 'A LIST parser that auto-detects MLSD, unix, DOS/IIS and VMS, handles wrapped VMS records and block-to-byte sizes, rolls back year-less unix dates, and is not fooled by a user named "jan".' },
      { category: 'added', text: 'WebDAV on node:http rather than fetch, because fetch cannot carry a client certificate or a TLS floor without reaching into undici internals. Namespace resolution treats D:, d:, a: and a defaulted xmlns as the same element.' },
      { category: 'added', text: 'S3 SigV4 signing written against node:crypto with no AWS SDK; every published AWS vector reproduces exactly, and the canonical request and string-to-sign are asserted as well as the final signature.' },
      { category: 'security', text: 'Certificate verification prompts before any credential is sent, and a refusal closes the connection.' },
      { category: 'fixed', text: 'Multipart upload aborts on failure, including CompleteMultipartUpload failing inside an HTTP 200.' },
    ],
  },
  {
    ...
    refs: ['#2', '#3', '#4', '#5', '#6', '#11'],
    changes: [
      { category: 'added', text: 'SFTP and SCP over ssh2 sharing one transport: authentication, tunnels, proxies, listing, and transfers that resume from an offset.' },
      { category: 'added', text: 'FTP/FTPS with MLSD plus a LIST parser that survives unix, DOS/IIS and VMS listing dialects.' },
      { category: 'added', text: 'WebDAV written by hand against fetch, with PROPFIND parsing across server namespace dialects and Digest auth verified against the RFC 7616 worked examples.' },
      { category: 'added', text: 'Transfer queue, synchronization, file masks, recursive find, session management, IPC, logging, editors, custom commands and Git-backed history.' },
      { category: 'fixed', text: '`npm test` was quietly broken: `node --test test/` made Node 26 look for a module called "test" and fail before running a single assertion. With a glob, 89 tests run and 88 pass.' },
    ],
  },
  {
    ...
    refs: [],
    changes: [
      { category: 'added', text: 'The control inventory is extracted from WinSCP’s own .dfm files rather than described from memory: 48 dialogs, 2,982 controls, 1,059 of them interactive, with captions, types and tab sheets.' },
      { category: 'added', text: 'All 301 WinSCP actions with captions, hints, categories, 79 shortcuts, side (local/remote/current/both) and focused flags.' },
    ],
  },
  {
    ...
    refs: [],
    changes: [
      { category: 'added', text: 'The Adapter contract every file-system backend implements, so adding a protocol never touches the UI.' },
      { category: 'added', text: 'The configuration store, and a coverage ledger that counts an unmapped unit as not started and a broken mapping as zero rather than as coverage.' },
    ],
  },
  {
    ...
    refs: [],
    changes: [
      { category: 'added', text: 'WinSCP’s own source is vendored read-only at vendor/winscp as the behavioural reference for the port.' },
    ],
  },
  {
